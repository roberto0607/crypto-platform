...
### ✅ Sub-panel hover + live-update refinements — shipped (PRs #83–#91, 2026-06-24)
*Polish on already-built indicators, NOT a new roadmap stage — the roadmap stays complete.*
- **Hover readouts** — ALL EIGHT trade-page sub-panels now have TradingView-style hover readouts (header switches `hovered ?? latest`, plus a synced vertical crosshair marker). Three variants by data shape: EXACT-match against the MAIN crosshair (data 1:1 with candles) — RSI, CVD, MACD (3-value — macd/signal/hist), ATR, Delta; STEP-lookup against the MAIN crosshair (value *in effect* at the cursor time, for sparse main-synced series) — Funding, OI; SELF-CONTAINED own-chart crosshair (`CrosshairMode.Magnet` snaps to weekly points, reads via `param.seriesData.get`) — COT (#91), which isn't main-synced (weekly UTC vs intraday) and so responds to hovering ITSELF, not the price chart.
...
- **Reusable patterns** (breadcrumbs): sub-panels are separate `lightweight-charts` instances. Three hover variants: (1) EXACT — subscribe to `mainChart.subscribeCrosshairMove`, look up by `param.time === point.time` (candle-aligned, `TZ_OFFSET_SEC`-adjusted so comparable); (2) STEP — same main subscription but last point with `time <= cursor` (sparse main-synced series); (3) SELF-CONTAINED — subscribe to the panel's OWN `subscribeCrosshairMove` + `CrosshairMode.Magnet`, read `param.seriesData.get(series)` (for panels on their own x-domain that can't main-sync). Live-update = recompute on `sse:price.tick`, throttled. Always guard range-sync until the panel has data.
- **PRs**: #83 (RSI hover), #84 (RSI live-update), #85 (CVD/MACD/ATR/Delta hover), #86 (Funding/OI step-lookup hover), #87 (range-sync guard), #89 (MACD/ATR live-update), #91 (COT self-contained hover).
- **Remaining trade-page polish** (tracked, not a roadmap): real-match replay enablement (see the Stage 6 limitation — needs a real match played + a per-trade event log).
