...
- **KNOWN dev-only non-issue (do NOT re-investigate or guard)**: the `Object is disposed` console error is a **Vite Fast Refresh / HMR artifact**, NOT a bug. It's thrown *inside* lightweight-charts (fancy-canvas `DevicePixelContentBoxBinding.get` → `tryCreateCanvasRenderingTarget2D` → `TimeAxisWidget._internal_paint`) — the library's own device-pixel ResizeObserver repaint firing after `chart.remove()` disposed the canvas binding, triggered by a hot-reload remount (amplified by React StrictMode) and using the dev-only `?v=` optimized-dep module that doesn't exist in a production Rollup build. A reproduction battery (indicator on/off, collapse/expand, toggle-while-pan, pair/timeframe switch, tick-flood, resize-during-dispose) was ALL NEGATIVE on a fresh page — it does not reproduce from any user action, and production is unaffected. Our cleanups are correct (rAF guarded by `chartRef.current?`, ResizeObserver `disconnect()` before `remove()`, hook unsubscribes crosshair handlers before `chart.remove()`). A guard would swallow a phantom — leave it alone.
