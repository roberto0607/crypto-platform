...
echo "=== 9. Large MARKET BUY → rejected (insufficient_liquidity) ==="
LARGE_HTTP=$(curl -s -o /tmp/sim-large-resp.json -w "%{http_code}" -X POST "$BASE/orders" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"pairId\":\"$PAIR_ID\",\"side\":\"BUY\",\"type\":\"MARKET\",\"qty\":\"99999\"}")
LARGE_CODE=$(cat /tmp/sim-large-resp.json | python3 -c "import sys,json; print(json.load(sys.stdin).get('code',''))" 2>/dev/null || echo "")
check "Large MARKET BUY rejected 400" "$([[ "$LARGE_HTTP" == "400" ]] && echo true || echo false)" "http=$LARGE_HTTP code=$LARGE_CODE"
