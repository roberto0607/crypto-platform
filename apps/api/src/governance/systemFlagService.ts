...
export async function isPairTradingEnabled(pairId: string): Promise<boolean> {
  const result = await pool.query<{ trading_enabled: boolean }>(
    `SELECT trading_enabled FROM trading_pairs WHERE id = $1 LIMIT 1`,
    [pairId],
  );
  if (result.rows.length === 0) return false;
  return result.rows[0].trading_enabled;
}
